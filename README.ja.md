...
- **録画がマシンの外に出ることはありません。** 録画は `~/.cc-wire-analyzer/` にプレーンな JSONL として書き出され、トラフィックは CC が元々使っていた上流にそのまま転送されます。テレメトリもアカウントもアップロードもありません。本アプリ自身の外部通信は 3 つだけで、いずれもクリックしたときのみ発生します：詳細ビューの任意機能である翻訳 / AI 解説（選択した内容を**あなたが**設定したエンドポイントへ送信）、情報パネルの「更新を確認」（api.github.com に最新リリースの tag を問い合わせるだけで、あなたに関する情報は送りません）、そして続けて「ダウンロード」を押したときに GitHub からその成果物を取得すること。**定期的な自動確認もサイレントインストールもありません**——すべて手動の一手です。
...
[Releases](../../releases) から最新の `cc-wire-analyzer-v<バージョン>-windows.exe` または `cc-wire-analyzer-v<バージョン>-macos.zip` を取得。Python は不要。バージョンはファイル名にもファイル自身のプロパティにも入っています（Windows は「詳細」タブ、macOS は「情報を見る」）。**開かなくても手元のビルドを見分けられます。** 併置の `SHA256SUMS.txt` は、アプリ内アップデートがダウンロードを検証するための一覧です。
