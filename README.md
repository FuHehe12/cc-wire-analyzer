...
从 [Releases](https://github.com/Fuhehe12/cc-wire-analyzer/releases/latest) 下载对应平台的安装包：

- **Windows**：下载 `…-windows.exe`，双击即可运行，无需安装。首次运行若被 Windows SmartScreen 拦下，点「更多信息」→「仍要运行」。
- **macOS**：下载 `…-macos.dmg`，双击打开，把应用拖入「应用程序」文件夹。首次打开如提示无法验证开发者，到「系统设置 → 隐私与安全性」点一次「仍要打开」。同名的 `.zip` 是应用内更新器使用的格式，首次安装请用 `.dmg`。

安装后的更新都在应用内完成：点「检查更新」，下载后一键换版并重启。
