...
3. 同一 `goal` 条目中的 `goal_flow` 承载整个 A→G。A 是最初输入时执行 AI 的理解，anchor 只记一次用户原话、当时理解与自主取舍；未经用户确认的自主取舍标“值得复核”。记录初始 G 后，仅在目标、范围、结束标准或分叉有实质变化时追加迭代，写清修改者、前后差异、触发因素与证据。用户调整、AI 自改、用户发起而 AI 定形态分别记录；后续新指令不能新建 A。工具调用、换文件或错字修订不自动构成目标变化。原话与反推分开，不把推断当事实；保留已有 anchor 和迭代，纠偏、验收及终止用有证据的后续迭代记录。
...
5. 写给非程序员看：标题用完整句解释动作与结果，目标标题直接呈现 AI 的理解；正文区分原话、解释和推断，必要的工程细节退到证据层。每项结论能回到具体 req_ 证据。`progress=done` 仅表示工作结束，不等于目标验收通过；工具成功或 AI 自称完成都不能代替用户验收或独立核验。未知、失败和未达成的工作照实保留。
...
观察时先问：执行 AI 最初把要求理解成了什么，后来的哪句话或哪项发现改变了它实际追求的目标？A 固定，G 随实质变化演进；一次新指令可以切换目标主体，也可以不改变目标。初始理解中的自主取舍、用户有歧义时 AI 自行选定的解释，都需要显式留下供用户复核。引用是录制原话，反推是外环判断，不能混写。

再问：为了当前目标，它做了哪几块工作，改变了什么，有什么核验或遗留？工作块按意图持续归组，不能把数百次读文件直接画成数百阶段，也不能为压缩而丢掉改变、决定与失败。能确定归属时把工作块和发现等关联到明确的目标迭代；不确定就保留空关联。发现是过程中的测量点，只有确实触发目标变化时才成为新迭代的原因。小幅文案修订用普通条目更新，不伪装成目标演变。

最后问：当前目标真的通过验收了吗，证据来自哪里？完成一个工具调用、结束一段工作和满足用户目标是三件不同的事。分叉的工作须保留各自结果，切换目标也不能抹掉旧目标的未达成部分。现场观测与事后复盘都可以有价值，但必须如实说明证据覆盖范围与判断产生的时间，不能事后制造前瞻命中。
