...
4. **三类变化并列，别只在前两类里二选一**（260912 反馈实测）。同任务交付结果、范围约束或验收标准实质改变称“修正”（`refine`）；同一整体目标下开始另一件可独立交付的任务称“转折”（`turn`）；**整体目标本身换了称“目标转换”（`goal`）——用户提出新的结果诉求，或把前面做过的事重新框定成更大目标的子步骤**，例如从“精炼这份 CHANGELOG”到“把整个项目转到原型阶段：先清掉测试期材料，再多轮审查”。三者里只有 `goal` 另起一个整体目标。不是看见“另外”、换工具或换文件就分段，原因假设更新、摘要续接与历史重发也不制造新目标。同会话只有一个 A，可以包含多个任务。任务转折保留旧任务未完部分和持续约束，不自动表示前一个任务已完成。**用户提出新要求时，先复核整体目标是否跟着变了，再决定记哪一类**；默认它是子交付正是首版会翻车的地方。
...
`tasks:[{id,title,start_id}]` 显式声明任务（T），start_id 指向该任务的首个迭代。迭代的 `task_id` 与 `change` 成对使用：initial 是首个显式任务的首个迭代；refine 是已有同任务的修正，所有父边指向同任务；turn 是新任务的首个迭代，至少一条父边指向已知不同任务；goal 的结构条件与 turn 相同，语义是**整体目标本身变了**。

**G 与 T 是两个平面**（260909 用户口径）：G 是对话中的最高目标，T 是 G 的内环——为达成 G 拆出的交付单元。**「G」一词只留给整体目标**，任务级一律说 T 或“该任务的首个迭代”，两边都叫 G 会把 `turn` 误读成整体目标动作（260912 反馈实测的诱因之一）。换一件交付用 `turn`，整体目标不变、不产生新的 G；只有用户提出新的结果诉求时才用 `goal` 另起一个 G。同任务口径调整用 `refine`。
...
别把「又开了一件事」当成「目标变了」，那会让一场会话里凭空多出十来个 G；也别因为怕多造 G 就把真正的目标跳变一律压成 `turn`——260912 实测里，观察者正是被单边警示推着把两次真实跳变记成了 `turn`，用户退回后才重建。**记 `goal` 时在该迭代的 trigger 里写一句「为什么是 goal 不是 turn」**：`turn` 与 `goal` 的结构条件完全相同，只差语义，写不出这句理由通常说明它其实是 `turn`。
