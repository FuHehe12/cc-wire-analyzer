...
4. **先判最高目标变没变，再判是哪一类**（260912 反馈实测）。**G 是这场对话里执行 AI 当前认定的最高目标**（界面标签写“整体目标”，与本文的“最高目标”同指一件事），判据一句话：*它现在做的每一件事，最终是为了什么*——答案就是 G。它不是用户原话的抄录，也不是你事后归纳出来的主题。**T 是在 G 不变的前提下拆出的交付单元**，判据同样一句：*这件事做完或放弃，最高目标还是那个吗*——还是，它就是 T。三类变化并列，别只在前两类里二选一：同任务的交付结果、范围约束或验收标准实质改变记“修正”（`refine`）；同一最高目标下开始另一件可独立交付的任务记“任务转折”（`turn`）；**最高目标本身变了记“目标变化”（`goal`）**，三者里只有它另起一个 G。判断固定三步、不要跳步：①拿“最终是为了什么”对着新要求复核一遍，先问 G 变没变；②变了就记 `goal`，并在该迭代的 trigger 里写清**它是改进还是转折、为什么不是 turn**；③没变，才在 `refine` 与 `turn` 之间二选一。不是看见“另外”、换工具或换文件就分段，原因假设更新、摘要续接与历史重发也不制造新目标。同会话只有一个 A，可以包含多个任务；任务转折保留旧任务未完部分和持续约束，不自动表示前一个任务已完成。**默认新要求是子交付，正是首版会翻车的地方。**
...
8. **建模模式要声明，误目标要留痕**（260909 实测结论）。实时跟随一律逐轮增量提交，`goal_flow.mode` 写 `incremental`：每批先更新 current，目标实质变化才动 G。事后复盘允许一次性建模，但 `mode` 必须写 `retrospective`——一次性复盘会把「错目标 → 改目标」压成 refine 的一句 trigger，那个曾经被相信、做完、甚至自检通过的目标，在 G 序列里就从未存在过。已知被推翻的中间目标，用 `status:"mistaken"` 事件挂在那个 G 上，写清怎么发现的并附证据；`superseded` 说的是被后来的目标接替，不是这个意思。两者判据就一句：**随录制增长分批提交写 `incremental`；把已经成段的整场录制一次性通读重建写 `retrospective`**，跟“建的时候录制还在不在长”走，不看你读了多少轮。

9. **整体目标可能起初就不存在，这也要如实记**（260912 反馈实测）。用户常常一开始没说真实目的，甚至自己也还没想清楚：中途一连串要求各自成立、彼此关系不大，真实目的到很后面才浮现。这时 `turn` 与 `goal` 的判据同时失效——没有稳定的整体目标，就无从判断新要求是它的子交付还是把它换掉了。**不要事后硬造一个贯穿全程的连贯目标**，那是复盘失真的另一种形态。做法：`current.understanding` 首句固定写“当前整体目标：…”，尚未明确时就直写“用户尚未说明真实目的，目标逐步显现”，并在目的浮现那一轮追加 `correction` 事件说明前面几段为什么各自记成 `goal`。真实样本：2026-09-12 会话 `9287e1ca`，可见记录 #26–#35 出现七次要求跳变（精炼 CHANGELOG → 改工作区规范 → 删测试材料 → 合并栏目 → 删打包文档 → 重组 issue → 为原型阶段做准备），真实目的到 #35 才说清；首版压成一个总目标，被用户退回后重建成三处 `goal`。

### 当前任务与增量写入

`tasks:[{id,title,start_id}]` 显式声明任务（T），start_id 指向该任务的首个迭代。迭代的 `task_id` 与 `change` 成对使用：initial 是首个显式任务的首个迭代；refine 是已有同任务的修正，所有父边指向同任务；turn 是新任务的首个迭代，至少一条父边指向已知不同任务；goal 的结构条件与 turn 相同，语义是**最高目标本身变了**（改进与转折都算，见下）。

**G 与 T 是两个平面**（260909 用户口径）：G 是这场对话里执行 AI 当前认定的最高目标——「它现在做的每一件事，最终是为了什么」，答案就是 G；T 是 G 的内环——在 G 不变的前提下为达成它拆出的交付单元，「这件事做完或放弃，最高目标还是那个吗」，还是就是 T。**「G」一词只留给最高目标**，任务级一律说 T 或“该任务的首个迭代”，两边都叫 G 会把 `turn` 误读成最高目标动作（260912 反馈实测的诱因之一）。换一件交付用 `turn`，最高目标不变、不产生新的 G；同任务口径调整用 `refine`。

**最高目标本身也会变，而且有两种变法，两种都记 `change:"goal"`**（260912 用户口径）：

| 变法 | 它长什么样 | 例子 | 会被误记成 |
|---|---|---|---|
| **改进** | 最高目标仍指向同一件事，但被抬高、收窄或加了新的结果条件；方向是连续的 | “原来要 X，现在要的是能自动出具的 X”；“原来是精炼这份 CHANGELOG，现在是把整个项目转到原型阶段，前面那件成了它的第一步” | `refine`——因为看起来只是把目标说得更准 |
| **转折** | 要的结果换成了另一件，旧的最高目标被放下或降级 | “这批报告先别管了，我们现在要把整个流程转成自动出具” | `turn`——因为结构上它确实也开了一件新交付 |

两种都另起一个 G，结构规则完全一样，差别写进 trigger。**改进型是更容易漏掉的那一半**：没有“换了一件事”的外部信号，方向又连着，于是回落成 `refine`；判据仍是那一句——复核完“最终是为了什么”，答案已经换了，就是 `goal`。旧 G 用 status 事件收口：被后来的目标接替写 `superseded`，没做完就被放下写 `unresolved`，方向本身错了写 `mistaken`。

**注意术语撞车**：`turn` 的中文标签就叫“转折”，但那是**任务层**的转折——换一件交付、最高目标没动；用户说“目标转折”指的是上表第二行，落点是 `change:"goal"`。写的时候把层说出来（“任务转折”/“最高目标转折”），不要留裸的“转折”。

判据两边都要给，别只记住一半：

| 不是 goal | 是 goal |
|---|---|
| 「另外再帮我…」「顺便把 X 也做了」——同一目标下又开一件交付，记 `turn` | 「我们现在是在为 X 做准备」「接下来整体要做的是 Y」——用户给出新的结果诉求 |
| 换工具、换文件、换目录 | 把前面做过的几件事重新框定成更大目标的子步骤 |
| 原因假设更新、摘要续接、历史重发 | 项目/工作本身换了阶段或性质（如“这个项目之前是测试，现在是原型”） |

别把「又开了一件事」当成「目标变了」，那会让一场会话里凭空多出十来个 G；也别因为怕多造 G 就把真正的目标跳变一律压成 `turn`——260912 实测里，观察者正是被单边警示推着把两次真实跳变记成了 `turn`，用户退回后才重建。**记 `goal` 时在该迭代的 trigger 里写一句「它是改进还是转折、为什么是 goal 不是 turn」**：`turn` 与 `goal` 的结构条件完全相同，只差语义，写不出这句理由通常说明它其实是 `turn`。
旧无任务标记的历史可以保留，不自动迁移、补标或推断 IDs。新任务不追加旧任务“完成”事件，除非另有验收依据。

例如“检查报告”→“报告还需核对图表”是任务 report 的 refine；“再写操作指南”是新任务 guide 的 turn；而“这批报告先别管了，我们现在要把整个流程转成自动出具”是 goal——用户要的结果本身换了，指南与报告都成了新目标下的旧事。若只是把缺图原因从模板改判为数据缺失，目标不变，只更新 current.situation。current 的 goal_ids 显式引用当前关注的 G，understanding/situation/evidence 必填；carryover 可记录“报告核对尚未完成，指南已开始；后续仍用日常语言说明”。不要从目标状态自动猜当前关注点。
...
**`[用户]` 里还混着同伴会话的话，一条规则认出来**（260912 实测）：正文以 `Another Claude session sent a message:` 开头、随后是 `<teammate-message teammate_id="…">` 的，是**别的 CC 会话发进这条泳道的消息**（多 agent 协作时的空闲通知与审查汇报），不是真人用户，也不是子代理泳道里的 `[派生指令]`。它照样占一个轮号，所以“用户又提了一轮要求”的直觉在这里是错的：`9287e1ca` 会话里 46 条 `[用户]` 行有 28 条属于这类（`req_6729a84` #36、`req_56a702c` #37、`req_0c576d9` #38 等，teammate_id 为 code-leftover / doc-hierarchy / fact-check 等审查代理）。把它们当作用户诉求会凭空造出目标变化；引用时保留来源，写成“同伴 X 汇报…”。
