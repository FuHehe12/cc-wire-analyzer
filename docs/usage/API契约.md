...
**两个平面要分清（260909 用户口径）**：G 是对话中的最高目标——用户总体上要达到的结果；T 是 G 的内环，为达成它拆出的交付单元。换一件交付是 `turn`，**不产生新的 G**；只有出现新的结果诉求、整体目标本身改变，才用 `goal`。同一任务内的口径修正是 `refine`，它既不是新 G 也不是新 T。导出页据此分两层：G 段在外，T 区在内，卡片按 `T<第几个任务>·<第几版口径>` 编号，不再把每条迭代都编号成 G。叙述里「G」只指整体目标，任务级说 T 或「该任务的首个迭代」——两边都叫 G 会诱导把 `turn` 当成整体目标动作（260912 实测）。
...
| `goal` | 结构条件与 `turn` 完全相同 | **整体目标本身变了**：用户提出了新的结果诉求（“我们现在是在为 X 做准备”“这个项目之前是测试、现在是原型”），或把前面做过的几件事重新框定成更大目标的子步骤。它同时开一个新任务，所以结构规则不变，区别只在语义——因此提交时在 trigger 里写一句“为什么是 goal 不是 turn”，写不出来的通常就是 `turn` |
