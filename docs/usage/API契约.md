...
**两个平面要分清（260909 用户口径）**：G 是这场对话里执行 AI 当前认定的最高目标（界面标签写“整体目标”，同指一件事）——判据是「它现在做的每件事最终是为了什么」，按录制里它当时表现出的理解记，不是用户原话的抄录，也不是外环事后归纳的主题；T 是 G 的内环，在**最高目标不变的前提下**为达成它拆出的交付单元——「这件事做完或放弃，最高目标还是那个吗」，还是就是 T。换一件交付是 `turn`，**不产生新的 G**；只有最高目标本身改变才用 `goal`。同一任务内的口径修正是 `refine`，它既不是新 G 也不是新 T。导出页据此分两层：G 段在外，T 区在内，卡片按 `T<第几个任务>·<第几版口径>` 编号，不再把每条迭代都编号成 G。叙述里「G」只指整体目标，任务级说 T 或「该任务的首个迭代」——两边都叫 G 会诱导把 `turn` 当成整体目标动作（260912 实测）。

**整体目标可能一开始不存在**：用户未说明真实目的、要求连续跳变时，`turn` 与 `goal` 的判据同时失效。结构上不额外提供字段，按提示词口径处理——`current.understanding` 首句写明当前整体目标，未成型就直写「用户尚未说明真实目的，目标逐步显现」，目的浮现时追加 `correction` 事件；不要事后补造一个贯穿全程的连贯目标（260912 反馈样本 `9287e1ca`：首版压成单目标，重建后为三处 `goal`）。
...
| `turn` | 新任务的首个 G；至少一条父边指向已知其他任务的 G | 同一个最高目标下开始另一项可独立交付的任务，并保留跨任务来路。注意这里的“转折”是**任务层**的，最高目标没动 |
| `goal` | 结构条件与 `turn` 完全相同 | **最高目标本身变了**，两种变法都用这一个取值：**改进**——仍指向同一件事，但被抬高、收窄或加了新的结果条件（“原来要 X，现在要能自动出具的 X”；把前面做过的几件事重新框定成更大目标的子步骤）；**转折**——要的结果换成另一件，旧目标被放下（“这批报告先别管了，现在要把整个流程转成自动出具”）。结构上它同时开一个新任务，所以规则与 `turn` 不变，区别只在语义——因此提交时在 trigger 里写一句“它是改进还是转折、为什么是 goal 不是 turn”，写不出来的通常就是 `turn`。改进型没有“换了一件事”的外部信号，是最常被误记成 `refine` 的一半。旧 G 的收口走 status 事件：被接替 `superseded`、没做完就被放下 `unresolved`、方向本身错了 `mistaken` |
