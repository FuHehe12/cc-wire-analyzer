...
- **实时分析页空态说明改为先讲用户处境，再讲机制**（issue 260912_实时分析空态文案讲清用途）。`ob.empty.body` 是这一页唯一的自我介绍位，原文案通篇讲机制（外环是另开的一个 AI、CCWA 只给事实存结论），第一次打开的人知道「谁在干活」，不知道「我为什么需要它」。改成三段式：vibe coding 多轮之后记不清跟 AI 说过什么、更难判断它把话理解成了什么（尤其它自创项目黑话）→ 这一页让外部 Agent 读录制、梳理整体目标，看清做过什么、在做哪一件、它怎么理解 → 原有机制交代与上手句。四处同步（行内兜底 + 中/英/日词典），`ob.empty.title` 不改（空态标题说状态，用途归正文）。
...
- **CHANGELOG 的写作要求移入 `CLAUDE.md`，并改成「条目求短」**（issue 260912_CHANGELOG写作要求移入CLAUDE）。要求此前分散在两处：`CHANGELOG.md` 开头的导语（分类与「说明问题、改动与必要影响」）和 `CLAUDE.md` 迭代纪律（「不设字数上限」）——写在被写文件自己开头的那半份在干活时读不到规则的身份，另一半还直接授权了长条目，于是条目越写越长（用户 260912 第二次点出）。现在导语只留导航并指向 `CLAUDE.md`，写作要求全部归「迭代纪律」，并把字数条款改成一条三到五句、细节留 issue 与 commit。存量条目不回改，新条目按新规则写。
