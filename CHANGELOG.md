...
- 实时分析首页先呈现“做了什么、产生什么结果、还剩什么”，每组先显示三条，剩余条目可展开；目标变化显示当前分支及原目标、调整后目标、触发原因和证据。关系图与预测默认折叠，保留原步骤下钻；旧观测未记录理解过程时明确提示，不从编辑历史推断目标变化。三语接入说明引导观察者按工作意图归组、使用完整句表达结果，语义分析仍由外部观察 Agent 完成。

### 验证

- 17 项 Python 自测、17 条观测阅读 Node 检查及相关静态检查通过。两组隔离浏览器回归覆盖原有阶段/预测阅读与新增目标变化，验证三语、三主题对比度、390/768/1280 宽度、证据下钻和刷新保留选择。体验包对话只用于本地复盘，未发布；尚未完成长时常驻外环的现场验收。
