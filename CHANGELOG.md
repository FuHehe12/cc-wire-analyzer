...
- **实时分析页空态说明改为先讲用户处境，再讲机制**（issue 260912_实时分析空态文案讲清用途）。用户反馈：这段文案（`ob.empty.body`，是这一页唯一的自我介绍位，`docs/` 无任何引用方）通篇只讲机制——外环是另开的一个 AI、它读录制判断 agent 在干什么、它不在这里跑、CCWA 只给事实存结论——第一次打开的人读完知道「谁在干活」，不知道「我为什么需要它」。真实处境是 vibe coding 聊了多轮之后**记不清自己跟 AI 沟通过什么**，更**难判断 AI 把自己的话理解成了什么**，尤其当 AI 术语越来越多、还自创了一堆只有它自己在用的项目黑话；要的是让外部 Agent 读 CCWA 里的信息、梳理整体目标，据此看清这场对话做了什么、现在在做什么、AI 的理解是什么。改动：正文重写为三段式（处境 → 这一页给什么，落在「整体目标 + 做过什么 / 在做哪一件 / 它怎么理解」 → 保留原有机制交代与「复制接入说明」上手句），四处同步（行内兜底 + 中/英/日词典），顺带消掉行内与中文词典此前的标点分叉（「，CCWA」vs「：CCWA」）。`ob.empty.title` 不改——空态标题说状态，用途归正文。验证：`check_i18n_js.py` 三语 698 键齐、`doc_audit.py` 对账干净、`check_refs.py` 未解析 0、`check_render.py` 全过；隔离 `CCWA_HOME`+`CCWA_CLAUDE_SETTINGS` 起本地服务实开三语看过空态卡渲染（卡片无固定高度，中文两行、英日各 142px，无溢出截断）。
