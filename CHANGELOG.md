...
- **当前状态**：v0.4.30 已发布。实时分析聚焦 A→G：先呈现执行 AI 对要求的理解与对现状的判断，再查看任务内修正、独立任务转折和原话证据。`public/` 仍冻结，发版不更新它。
- **下一步**：收集升级后的真实使用反馈，继续改善理解偏差的呈现与任务边界识别。外部观察 Agent 负责解释，程序不自动判断语义正确或目标达成。
