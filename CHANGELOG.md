...
- **Today's recording is compacted as it goes, instead of waiting for tomorrow.** `compact_date`
  refuses to touch today on purpose — `append` only ever writes today, and compacting it would mean
  contending with the write path for the same file — so a 28x compaction ratio was worth nothing on
  the day that actually fills the disk, and a single day here measures **500–860 MB**. Recording now
  seals the finished prefix into `{date}.pNN.pack` segments once it crosses a threshold, and merges
  them back into one `{date}.pack` on the following day. The line that mattered is intact: a segment
  is sealed by **renaming the tail under the lock, verifying it byte-for-byte outside the lock, and
  only then putting it in place** — the source file is still deleted only after verification passes,
  and a failed compaction restores the day to exactly the state it had before the cut. Historical
  days keep the format they already had (`PACK_SCHEMA` unchanged), so archives, imports and
  cross-machine transfers behave as before. Off by default; the settings page has the switch and the
  threshold (20–2000 MB, default 200). Measured on the self-test fixture: two segments plus a tail
  merged to **701 KB → 36.8 KB (19.1x)**, byte-identical to the original recording on unpack.
- **Archiving a day that had been rolled would have silently dropped everything but the last
  segment.** `archive_date` compacted `{date}.jsonl` directly, which under the new layout is only
  the tail — and archiving deletes the original recording by default. Found while fact-checking a
  sentence in the issue notes that claimed the opposite. The restore path is now one shared function
  used by both merging and archiving, because two copies of it is exactly how this happened.
- **The instructions you hand to an external agent now open up the analysis the app has already
  done.** The endpoint list went from 3 to 8 — `trajectory`, `subagents` and `analysis` were missing
  entirely — and it now names the fields `trajectory` has already computed (`loops`, `subagents`
  with their `trigger_step`, `phases`, `user_events`, `materials`/`provenance`, `verify`/`valves`,
  `cost`, `optimal`). Not naming them was the same as not shipping them: an agent will not guess
  which keys a JSON has, so it was re-deriving "where did it retry" from the raw text while the
  answer sat one endpoint away. A second task (`?task=flow`) asks for a diagram of how the run
  actually went. It does not prescribe how to draw one; it says what a flowchart means here — a
  production line the agent grew as it ran, read through three questions (how many state changes,
  what backs each artefact, what controls the flow) — and leaves the drawing to the agent. The brief
  also reports whether this snapshot has been summarised, so a mechanically-cut `phases` is never
  mistaken for an analytical conclusion.
