...
Claude Code 看起来还在工作，但自动模式明显变慢了。不必猜，你把本地 API 指给一个 agent：

> 读 `http://127.0.0.1:<端口>/api/ai-guide`，然后查清楚自动模式为什么变慢，以及是否有隐藏的安全分类器调用失败。
...
根因是一处配置矛盾：`settings.json` 顶层写着 `effortLevel: low`，而环境变量设了 `CLAUDE_CODE_EFFORT_LEVEL: max`——环境变量胜出。CC 自己的视图里没有任何迹象，真正的线索来自失败的辅助调用。一次 agent 调用，一个答案——不用肉眼盯时序图，不必复现那一刻。
