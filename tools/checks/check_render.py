...
用法：
    uv run python tools/checks/check_render.py            # 审计
    uv run python tools/checks/check_render.py --self-test # 造反例（NH_AGG→62）验证它真会报
"""
import re
import sys
...
INDEX = Path(__file__).resolve().parents[2] / "src" / "templates" / "index.html"
