"""文档对账闸门：把**代码里的事实**与**文档里的说法**摆在一起，不改任何东西，但会挡发版。
    ...
    uv run python tools/checks/doc_audit.py            # 人看；有硬差异则退出码 1
    uv run python tools/checks/doc_audit.py --json     # 给 agent；数据里看 `ok` 字段
...
`docs/development/开发约定.md` 第十一节 自己的判据就是**需要人工定期同步的药方自己就是下一处腐化**。
`.github/workflows/release.yml` 在 PyInstaller 之前跑它，fail-fast。

为什么存在（`docs/development/开发约定.md` 第十一节 策略五挂了很久的"待实现"）：
...
# 当前规范按职责分类；public/ 是冻结披露，local/ 与 issues/ 是过程资料，均不作为当前规范。
DEVELOPMENT = DOCS / "development"
USAGE = DOCS / "usage"
GUIDE = DEVELOPMENT / "开发约定.md"
REQUIRED_DOCS = [GUIDE, DEVELOPMENT / "架构总览.md"] + [
    USAGE / name for name in ("AI_USAGE.md", "API契约.md", "界面导览.md", "报文解读.md")]
...
# 文档面：当前工作文档中所有端点/命令/路径声明（本地 CLAUDE.md 与 issues/ 不进对账——
# 它们是过程记录，允许留下当时的说法）。
...
# **别的工具的端点**。文档里出现它们是正常的——`docs/guides/同类工具构建手册.md` 的主题
...
def _changelog_style(paths=None) -> list[str]:
    """检查两份必需的变更记录及全文机械硬折行，不限制正文长度。

    变更记录应解释问题、变化和影响，允许较长的完整段落。空行分段、表格、嵌套列表、
...
def _blockquote_style(paths=None) -> list[str]:
    ...
    **只查 docs/**：`research/` 装路线、判据、踩坑记录，是过程记录不是参考手册，
    允许自由文体；`docs/guides/` 的读者在本项目之外，同理不套。
    """
    out = []
    # guides/ 继承原 handbook/ 的文体边界，仅当前项目规范执行引用块样式规则。
    # guides 仍在 DOC_FILES 中，端点、枚举、路径和链接检查不变。
    for p in (paths if paths is not None else sorted(
            p for p in DOCS.rglob("*.md") if DOCS / "guides" not in p.parents)):
        lines = _read(p).split("\n")
        i, first = 0, True
        while i < len(lines):
...
def _spec_facts() -> dict:
    ...
    for name in _SPECS:
        ...
        #    全靠 `tools/build/version_res.py`；哪份 spec 掉了这行，那个平台的产物就退回
...
def audit() -> dict:
    ...
    # 1. 端点：代码有、契约没写。**只认 API契约.md**——开发约定 第十一节 指定它是端点的
    ...
    for name, text in doc_text.items():
        for m in re.finditer(r"`((?:src|tests|tools|docs)/[A-Za-z0-9_./-]+\.(?:py|md|html))`", text):
    ...
    # 5. 自测命令对应文件存在吗
    missing_selftests = []
    for m in re.finditer(r"uv run python ((?:src|tests|tools)/(?:[A-Za-z0-9_]+/)*[A-Za-z0-9_]+\.py)", _read_required(GUIDE)):
        if not (ROOT / m.group(1)).exists():
...
def _selftest() -> int:
    ...
    ecases += [
        ...
        # 「守卫函数存在但调用点缺失」。
        ("spec 掉了版本资源会挡发版",
         n_hard(spec_divergence=["build-mac.spec 没有引入 tools/build/version_res.py"]) == 1),
    ]
    # CHANGELOG 格式：长正文合法，机械折行必须拦截，缺失文件必须明确报错。
    import tempfile as _tf
    _cd = pathlib.Path(_tf.mkdtemp())
    _NL = chr(10)
    _good_en = "## Unreleased" + _NL * 2 + "- Retries now merge into the turn they retry." + _NL
    _long_en = "## Unreleased" + _NL * 2 + "- Requests previously lost their original error when a later retry succeeded. The recording now preserves both attempts and explains which response belongs to each attempt, so users can investigate the initial failure without mistaking the final success for the entire interaction." + _NL
    ...
    _long_zh = "## 未发布" + _NL * 2 + "- 之前重试成功后，界面只展示最后一次结果，用户无法理解最初为什么失败。现在保留每次请求及其对应响应，并明确展示重试关系，便于追溯原始错误及最终恢复过程。" + _NL
