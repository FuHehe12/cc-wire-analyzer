...
用法：
  uv run python tools/build/build.py                    # 读 src/_version.py，没有就是 "dev"
  uv run python tools/build/build.py --version 0.4.11   # 显式版本（也会写回 _version.py）
  uv run python tools/build/build.py --from-git         # 从 git describe --tags 推（版本真源是 tag）
  uv run python tools/build/build.py --self-test        # 验命名规则与 CI 一致，不真打包
...
def sync_version_file(version: str) -> bool:
    ...
    line = (f'VERSION = "{version}"'
            '  # CI 从 tag 注入，勿手改（见 docs/development/开发约定.md 第九节）\n')
    try:
