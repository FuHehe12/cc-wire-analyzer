...
权威位是否还在、分类是否还对得上。它不改任何分类逻辑，只把证据摆出来：
    ...
    uv run python tools/probes/lane_probe.py                    # 今天
    uv run python tools/probes/lane_probe.py --date 2026-07-14
    uv run python tools/probes/lane_probe.py --date 2026-07-14 --json   # 喂给 AI 分析
