...
const qa = path.resolve(process.env.CCWA_SITE_QA || path.join(root, 'local', 'artifacts', 'site-qa'));
...
const sections = ['read', 'flow', 'observe', 'compare', 'reading-assistance', 'agent', 'start'];
let server, browser;
...
(async () => {
  ...
  server = http.createServer((req, res) => {
    ...
    let file = path.resolve(root, 'dist', 'site', pathname.slice('/cc-wire-analyzer/'.length) || 'index.html');
    if (!file.startsWith(path.join(root, 'dist', 'site') + path.sep)) return res.writeHead(403).end();
    try {
