#!/usr/bin/env python3
"""Validate the assembled site in dist/site before any GitHub Pages deployment."""
...
MANUAL_SOURCE = ROOT / "dist" / "manual" / "index.html"
...
def check_manual() -> tuple[list[str], set[str], set[str]]:
    ...
    try:
        ...
        if not (b'<script id="bookData"' in source and b'<script id="data"' in source):
            raise ValueError("dist/manual/index.html is not a complete manual; run tools/build/build_manual.py")
        content = MANUAL.read_bytes()
        if content != source:
            errors.append("dist/site/manual.html differs from the built manual; run tools/site/build_site.py")
