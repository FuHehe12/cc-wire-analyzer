...
- **当前状态**：**main 上有未发版改动**——录制存储分了层：过去的天可以**原地压实**（内容寻址去重 + 逐块 zstd，实测 500 MB → 14.8 MB，33.9x，单条随机读 p90 27 ms，可逐字节还原）、**归档**成一个可搬运的 `.ccwa` 单文件、以及从另一台机器**导入**到 `sources/<标签>/` 这个独立命名空间。今天的录制一个字节都不动——写盘路径没有改。上一个发布版 **v0.4.14**（2026-08-10）把识别体系落成一张七层地图，并第一次拿 Claude Code 自己的对话记录当真值量了轮起源启发式（2,339 个轮一致率 99.8%，真人轮降档 0 例；`origin` 新增由 `cc_entrypoint` 判定的 `sdk` 档）。**macOS 升级用户注意**（自 v0.4.2 起未变）：应用包由 `CCWireAnalyzer.app` 改名 `cc-wire-analyzer.app`，`/Applications` 里旧的那份不会被替换，需自行删除。
- **下一步**：
