...
- **当前状态**：**v0.4.1 已发布**（2026-07-29）。在 v0.4.0 之上的一轮收口：**安全审查现在读得懂了**（在判什么动作、判成了什么、命中哪条规则、这次向上游发了什么——包括你的 CLAUDE.md 全文；`IDX_SCHEMA` 4 → 5，索引重建一次）、**v0.4.0 之后的代码复查**（一条永远不会触发的角色标注、一处没人会知道的分类失败、9 条死 i18n 键）、以及**一次文档体系重构**：开发约定原本散在三处且副本已分叉，现在只有一处（`docs/开发指南.md`），另新增 `docs/问题域手册.md` 给要换一个 agent 工具做同类分析器的人。三份 README 改为以「什么时候你会需要它」「它拿你的流量做什么」开场。
- **下一步**：
