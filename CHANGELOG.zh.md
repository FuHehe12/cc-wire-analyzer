...
- **下一步**：
  1. **失败聚合的跨天视角——已落地，并已修正。** `/api/diagnose/trends` 回答「新发还是老毛病」：每日曲线、趋势标记、新鲜度、host/model/cc_version 切片（HTTP + CLI，不进 GUI）。拿 12 天真实录制复查后修了「标签在说什么」这件事：单日 2650 次的事故原本被标成「零星」、两周前就停了的组仍读作「复发」、各供应商的空洞 `Error` 并成一个垃圾桶组。同一轮也修了盲区雷达（host 归属、按提升度算 beta 来源、本工具自己的降级标记单列）。仍未做：把反复出现的失败模式自动固化成体检规则——`effort_max_rejected_upstream` 当初是人工盯出来的复发，把这条闭环走通是剩下的另一半。
