...
- **当前状态**：**v0.4.4 已发布**（2026-08-01）。建立在 v0.4.3 之上的一轮可用性修复，三件事全部来自真实使用：软件开着过午夜会让界面冻在昨天（新录制是被静默丢弃，不只是"慢"）；小字颜色低于 WCAG AA，在 2K 屏上读起来发虚，已重新裁定阶梯并在设置里加了 90%~200% 的界面缩放；失败聚合面板从首屏横幅降级为日期行里的按钮——它是查问题时才要的归纳，不是告警。
  前一版 **v0.4.3** 是分量更重的那个：它收口了录制盲区审计，并终止了失败率的低报。三件最要紧的事：上游在 SSE 流**内部**报的错（HTTP 仍是 200）此前被录成**成功**，于是流内失败从来没进过失败统计——**本工具一直在低报上游失败率**；失败聚合终于有了界面入口，此前它当了六周项目自承的最大一道缝；面向 agent 的说明书现在打包在产物里（`--help`，以及运行后的 `GET /api/ai-guide`），只拿到一个可执行文件的机器上，AI 也能学会怎么驱动它。此外：录制现在覆盖 CC 声明的全部压缩格式（缺 brotli 曾让每个安全分类器响应的正文丢光）、CC 对自己的声明（`anthropic-beta`）录得下也看得见并成为盲区雷达、软件自报的版本号从此永远与 release tag 一致。
