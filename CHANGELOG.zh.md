...
- **当前状态**：**v0.4.6 已发布**（2026-08-01）。以「AI agent 会怎么用这工具」的方式实际
  驱动后做的一轮正确性修复：`grep` 只搜了请求的 14% 而且没法告诉你（现在覆盖全部区域，
  每次返回都声明覆盖面）；`stats` 的 token 汇总漏了 `cache_creation`（约占成本 38%）；
  `get` 不带 `--date` 时可能静默返回索引行冒充真记录；响应体解不开的请求在失败统计里
  被算成成功（现在单列 `decode_failed` 类别）；所有检查面——API 与 CLI——都能按会话
  过滤，第二个 Claude Code 经代理审计第一个时不再看见自己的流量。给人看的这边：慢日期
  加载时有加载徽标了；协议扩展基线不再对 beta flag 改名前的老录制误报。
  前一版 **v0.4.5** 是针对「一份录制里有多会话、有派生子代理」
  时序图的一轮正确性与可读性收口——让图与实际发生的一致。辅助调用（标题/安全/count_tokens）
  按会话 id 精确挂到所属主线，不再挂全局时序最近的那条；子代理泳道改用 CC 官方的
  `X-Claude-Code-Agent-Id` 头作键，泳道能直接 join 子代理自己的 jsonl 对话记录；关闭主线
  现在级联隐藏它派生的子代理链与辅助调用；子代理泳道改用派生方同色的淡化色，一族派生一眼
  可辨。另有几处来自真实使用的修复：快速切日期时 `loadDag` 的竞态可能让时序图停错天；
  「我把 max_tokens 改大、输出还是被截断」现在有了解释（三种成因、逐一指明）；编译版的
  复制/选中文本通了；`serve` 不再在还没有 settings.json 的机器上启动即退。
  前一版 **v0.4.4** 是一轮可用性修复——过午夜不冻、小字可读、90%~200% 界面缩放设置。
