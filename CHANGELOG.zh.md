...
> 定位 / 当前状态 / 下一步——AI 接手快照。仅作导航；属规则/不变量的关键判断见本地 CLAUDE.md（开发约定）。详细变更历史见下方各段。
...
- **当前状态**：**v0.3.2 已发布**（2026-07-19）。`未发布` 段是 2026-07-25/26 落地的三件大事：**泳道判别定案**（系统头计费位 `cc_is_subagent=true` 为权威信号，准确率 10/15→15/15）、**配置体检**（`doctor.py`，8 条只读规则）、**失败聚合**（`diagnose.py`，实测 2719 个失败→7 组）。六自测全绿，代码已重编译验证。待发版 v0.4.0。
- **下一步**：
  1. **发版 v0.4.0**（CHANGELOG 已就绪）。发版策略：打 tag 前先用真实流量验证无误报；打 tag 前重新打包 exe 冒烟。
  2. **诊断闭环**：① 失败聚合的 UI 入口（目前只有 API/CLI）；② 反复出现的失败模式固化成体检规则（`effort_max_rejected_upstream` 即由此而来——标准：可复现？能静态判定？误报风险？）；③ 跨天趋势（最远，暂缓）。
  3. **判别残余**（暂缓）：交互模式（`cc_entrypoint=cli`）的子代理**未实测**（本轮全是 sdk-cli）。两层规则保证 flag 缺席也不会再让主线被误判为子代理，但要彻底闭环需采集一次交互会话派生子代理的流量。
