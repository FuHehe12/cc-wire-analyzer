...
- **当前状态**：**v0.4.2 已发布**（2026-07-30）。一轮版式与打磨：详情页左右两栏重新对齐（一个裸 chip 行对上一张卡片，让两栏从第二块起全程错开 27px）、捕获列表改成固定列宽的真正表格（flex 让摘要列在 13 行里漂了 300px）、两个平台的 release 产物统一命名（`cc-wire-analyzer-{windows,macos}`，macOS 应用包一并改名，两平台 serve 命令写法从此一致）。建立在 v0.4.1 之上——那一版让安全审查读得懂了，并给了项目开发约定一个单一真源（`docs/开发指南.md`）。
  **macOS 升级用户注意**：应用包由 `CCWireAnalyzer.app` 改名 `cc-wire-analyzer.app`，`/Applications` 里旧的那份不会被替换，需自行删除。
- **下一步**：
  1. **失败聚合的 UI 入口已补上**（已完成，未发版的 v0.4.3）——这是项目连续六周自承的最大一道缝。它的输入也已可信：流内错误此前被录成成功、根本没进过失败统计，同在 v0.4.3 修掉。这条线上剩下的是**跨天**视角：趋势与复发，而不是单日。
