...
- 审子代理那次工具调用的安全审查，现在归到那个子代理，不再算主线。CC 在 wire 上把这件事
  藏起来了——所有辅助调用一律标成主代理——但它把被审 agent 的整段对话放进了请求正文，
  于是能用和子代理泳道**同一个**锚点认出来。7 天实测：556 条安全审查里 28 条属于子代理
  （子代理密集的日子占四成），其余辅助类型一条都不属于。轮卡计数、级联隐藏、token 成本
  跟着归到正确的一方。
- schema 变更后的索引重建，现在在 Windows 上真的会替换旧索引文件，而不是往后追加。此前旧文件
  被本进程自己的读句柄占着删不掉，于是每读一次就追加一整天——本机某天 196 条记录堆成 3,955 行、
  混着四代 schema。读出来的数据一直是对的，所以一直没被发现。已膨胀的文件在第一次读取时自动清理。
- 参考文档：五份全量通读体检，把它们自相矛盾与互相矛盾的地方逐条改对，删掉文章式的引子与
  收束，补丁章节号顺编。随产物打包给 agent 的那份说明书（`--help`、`GET /api/ai-guide`）也在其中。
- 时序图不再在一张折叠卡代表多个节点时画几十条完全重合的连线；辅助泳道也不再用「会话顺序」
  把分属不同会话、不同 agent 的辅助调用串成一条线。
- 时序图切换折叠模式时，辅助聚合卡的展开态也一起重置。此前手动点开一张聚合卡后，再点
  「展开所有轮」「按轮折叠」，那一组辅助仍是散开的单条——看上去像折叠按钮管不着辅助。
  原因是模式切换只清了轮卡的展开态，聚合卡是后加的，两次迭代都没接进来。
