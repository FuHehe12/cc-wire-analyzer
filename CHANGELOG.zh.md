...
- **当前状态**：**v0.4.15 已发（2026-08-25）**——录制存储分了层：过去的天可以**原地压实**（内容寻址去重 + 逐块 zstd，实测 500 MB → 14.8 MB，33.9x，单条随机读 p90 27 ms，可逐字节还原）、**归档**成一个可搬运的 `.ccwa` 单文件、以及从另一台机器**导入**到 `sources/<标签>/` 这个独立命名空间。今天的录制一个字节都不动——写盘路径没有改。未发版改动把两套浅色外观下仍是深色的白板与贴纸整族接入语义 token，并让「引用一个谁都没定义的 CSS token」从静默失效变成挡发版的硬差异。上一个发布版 **v0.4.14**（2026-08-10）把识别体系落成一张七层地图，并第一次拿 Claude Code 自己的对话记录当真值量了轮起源启发式（2,339 个轮一致率 99.8%，真人轮降档 0 例；`origin` 新增由 `cc_entrypoint` 判定的 `sdk` 档）。**macOS 升级用户注意**（自 v0.4.2 起未变）：应用包由 `CCWireAnalyzer.app` 改名 `cc-wire-analyzer.app`，`/Applications` 里旧的那份不会被替换，需自行删除。
- **下一步**：
...
- **列表视图升级为读得动的步级简报流。** 长会话（上百步、子代理、成墙的工具调用）让录制在体感上成了黑箱：旧的步级行是机械事实——chips 和字数，没有"这步在干嘛"。现在点「AI 归纳」会同时生成**每步一行简报**（原料=该步的思考+回复；40 字约束，模型编造的步号被与轮级归纳同一个白名单剔除），按每批约 8K 字符分批调用——上百步的会话只是几次小请求，结果并入同一份 analysis 缓存，重开零成本。纯工具步不进模型：连续的聚成一行降权簇（`#12-18 [Grep×6 Read×4] · 14 次工具调用`），要说的就这么多。视图本身零点击：思考原文的钻探留在树视图（列表=读故事，树=查证据），signals chip 继续作为 AI 措辞的机械对照，失败的批如实自陈（"2 批失败，对应步显示机械信息"）。真机验收：50 步 / 7.4 万字思考的真实录制，7 批全成功、47 条可读简报。
