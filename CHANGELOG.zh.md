...
- **当前状态**：**v0.4.7 已发布**（2026-08-02）。给人看的一面：时序图从「一串请求」变成「一串轮」——你说的每句话一张卡，子代理与辅助折在卡上；界面有了三套外观并默认深色。给 AI 用的一面：盲区雷达与跨天趋势拿 12 天真实录制复查后修正了标签语义（单日 2650 次的事故曾被标成「零星」），两者都补了 CLI，另加 `tools/doc_audit.py` 做代码与文档的机器对账。**macOS 升级用户注意**（自 v0.4.2 起未变）：应用包由 `CCWireAnalyzer.app` 改名 `cc-wire-analyzer.app`，`/Applications` 里旧的那份不会被替换，需自行删除。
- **下一步**：
