...
- **当前状态**：**v0.4.7 已发布**（2026-08-02）。这一版让两个面各自说了实话。给人看的这边，
  时序图不再是一串请求，而是一串**轮**：你说的每一句话一张卡，卡上带着这轮派生的子代理与
  触发的辅助调用，其余全部折进卡里（实测某天从 19 列 192 个节点降到 18 列 68 张卡）。
  界面也有了三套外观、默认改为深色——流量分析工具是跟 DevTools、终端一起看的，
  原来那套单一暖灰在其中显得格格不入。给 AI 用的这边，盲区雷达与跨天趋势拿 12 天真实录制
  复查后，发现它们的若干标签本身就是错的：单日 2650 次的事故被标成「零星」、两周前就停了的组
  仍读作「复发」、各供应商空洞的 `Error` 并成一个垃圾桶组，雷达报的 beta 来源实际上是当天的
  基线 flag 而非任何可疑的东西。两者现在也都有 CLI 子命令——审计的铁律是只读，而 `serve`
  会 patch 你真实的 settings.json。另加 `tools/doc_audit.py`：对账代码事实与文档说法之间
  六项机器可判定的东西，首跑就查出四处。
  前一版 **v0.4.6** 是以「AI agent 会怎么用这工具」的方式实际
...
- **下一步**：
  1. **把自迭代闭环走通：反复出现的失败应当自动固化成体检规则。** `/api/diagnose/trends` 现在能可靠地回答「新发还是老毛病」（v0.4.7 修正了它的标签实际在说什么），但从「一个复发模式」到「一条体检规则」仍然靠人——`effort_max_rejected_upstream` 之所以存在，是因为有人盯出了那次复发并手写了规则。把这半自动化，雷达与趋势才不只是「读一读」。
...
### 新增
- **三套界面外观，默认改为深色。** 界面从 v0.1.0 起一直是一套暖灰米金——好看，但流量分析工具是
  跟 DevTools、终端、编辑器一起看的，那几样没有一个是米色。设置页现在给三选一：**深色专业**
  （默认，`#131318` 炭底 + 驼金强调，多层阴影与内高光、玻璃态顶栏、报文区凹陷终端底）、
  **经典暖灰**（与 v0.4.6 完全一致，想保持原样的选它）、**实验室日光**（冷矿物纸面 + 青蓝强调
  + 24px 测量网格）。切换即时生效，选择写进 cookie + localStorage 并在首屏绘制前读回，因此没有
  深浅闪切；**绝不写进后端 `config.json`**——那份配置是代理行为的真源，外观不该掺进去。
  同一轮还补了比主题更早就欠着的可用性：toast 与状态标记的对比度、捕获行与时序节点的键盘激活、
  `label`/`switch`/live region/tab 语义、`prefers-reduced-motion`、窄屏与高缩放布局。
  泳道色板按外观分三份——同一组六色不可能既落在炭底上、又落在纸面上，还两边都过 AA。
  - 移植过程中暴露了本周期早些时候上线的轮卡的一个对比度 bug：子代理徽章的文字写死 `#fff`，
    而它的底色是算出来的（派生方色兑 45% 白），实测 **1.6:1**，等于看不见。现在前景色按底色亮度
    选（三套外观下 7.5~11.5:1）。**底色是算出来的，前景色就不能写死**——写死的那一刻它就在等着
    某套配色把它变成隐形字。
- **时序图改为按对话轮折叠，取代「隐藏工具调用」。** 原有的「隐藏工具循环步」让大流量天的图变短了，
  但没让它变得可读：过滤之后剩下的仍然是**请求**，而每张卡的主体是模型回答的前 60 字——于是
  「我那轮说了什么」恰恰是图上唯一看不到的东西，而那才是人回溯时的检索键。现在轮是一等公民
  （`build_dag` 返回 `turns`，分轮判据沿用既有的 `turn_start`），折叠态下 main / 子代理泳道每轮
  一张卡：**你的话是主体**，下面是这轮派生的子代理徽章（点一下直接聚焦那条泳道）与触发的辅助
  调用计数。点卡片展开该轮逐条请求。**辅助也跟着轮折叠**——折叠态下它们是卡上的计数而不是一行一行的节点，全折叠时 aux 列整列消失（实测某天从 19 列 192 个节点降到 18 列 68 张卡）；没有归属轮的辅助照常显示，藏了就是静默丢数据。两个开关都保留——折叠是聚合、隐藏是过滤，展开某轮后后者
  在该轮内部仍然生效。失败在卡上是 ⚠N，只有**整轮全挂**才整卡染红：31 步里 1 次瞬时 429 不等于
  这一轮失败（实测一天 68 轮有 29 轮含至少一次失败，一律染红等于把红色用废）。辅助调用只能归到
  主线的轮、归不到子代理：9 天 1290 条辅助请求里带 `X-Claude-Code-Agent-Id` 的是 **0 条**，而
  `session_id` 与父会话共用，wire 层没有任何标识能说「这次安全审查在审子代理的工具调用」——
  靠时序邻近猜与本项目「官方标识符优先」的定案相悖，不做。索引新增 `turn_user`
  （`IDX_SCHEMA` 14→15）：必须写时算，因为 `last_user` 只存前 2000 字，而 CC 注入的
  system-reminder 可达 9960 字，读时现剥常常剥出个空。
- **自动适应宽度不再缩到 50% 以下。** 19 条泳道的那天它算出来是 21%，那个尺寸下卡片上的字一个也
  读不出来——「适应宽度」成了假承诺。低于下限改为保持可读、让横向滚动；想更小仍可用 − 按钮。

- **CLI 补 `unknowns` 与 `trends`；所有只读面都接会话过滤。** 盲区雷达和跨天趋势落地时只有 HTTP
  一面，而本机自审工作流是 CLI 优先的——理由很实在：`serve` 会 patch 你真实的 `settings.json`
  （那是录制机制的一部分），而审计的铁律是只读。于是"看一眼这周积了什么未知"这件纯只读的事，
  偏偏卡在项目里唯一有副作用的那条路径上。现在两者都是 CLI 子命令，与路由调同一个函数
  （`capture_store.unknowns` / `diagnose.trends`）；`/api/grep`、`/api/stats`、`/api/unknowns`
  也终于认 `session` / `exclude_session`——v0.4.6 承诺的是"所有检查面都能按会话过滤"，这三个
  当时在承诺之外。`stats` 顺带改走索引：它用的字段全在索引里，而原先逐行 parse 主文件、每条还调
  `classify(完整 record)`（等于把整条 `index_record` 重算一遍，含拿 ~108K 安全规则库做匹配）。
- **`tools/doc_audit.py`——代码事实与文档说法的机器对账。** 文档维护策略把它挂在"待实现"很久了，
  而那份策略自己的判据就是：**给腐化开的药方如果需要人工定期同步，它自己就是下一处腐化**。
  它查六项机器可判定的事：端点有没有进 `API契约.md`（策略指定的端点真源，"某份文档提过"不算）、
  CLI 子命令有没有文档、文档指向的文件在不在、文档里断言的 `IDX_SCHEMA` 数值、自测清单里的文件
  在不在、有没有引用已删端点。只报差异、不下判断、退出码永远 0。首跑查出四处：`/api/grep` 与
  `/api/stats` 从没进过契约、17 个 CLI 子命令在公开文档里零清单、架构总览的 `IDX_SCHEMA` 还钉在
  10（代码 14）、`tools/check_i18n_js.py` 是写死绝对路径和某次键名的一次性脚本却摆在 tools/
  像通用工具（已重写）。
...
### 变更
- **雷达：三处"它到底指向哪"的修正。** 拿 12 天 5505 条真实录制复查得出。（1）每个未知现在带
  `hosts` / `cc_versions`。复查那天**全部 5 条未知**都来自同一个第三方网关（响应里回 OpenAI 风格的
  `tool_result` 块），而端点当时只说"协议演进、稳定的并入 `KNOWN_*`"。照着做等于**用某个网关的
  形状去放宽官方链路的判据**，将来官方真出现同名异构块，雷达反而哑掉。host 归属现在是判读第一步。
  （2）每项的 `betas` 改算提升度——`P(beta|带该未知的请求) ÷ P(beta|全体)`，只留 ≥1.5——不再用裸
  计数。裸计数报出来的永远是每条请求都带的那几个（实测那 5 个恰好是当日出现率 100%/96% 的前 5 个）；
  而只出现一次的未知，所有 beta 计数全平局，`most_common` 退化成"取 header 里的前几个"。现在**空
  列表是诚实的答案**。（3）本工具自己的降级标记（`_input_raw` / `input_raw_fallback`——SSE 在
  `content_block_stop` 前被截断、或工具入参拼不出 JSON）移到单独的 `degraded` 维度：它们说的是
  **这条录制的正文是残的**，该查代理侧，与协议无关；而有一整天上报的"未知"100% 是它们，真信号被
  自己的噪声顶掉了。
- **雷达：已知集合的覆盖面。** `compaction`——本工具自己的 SSE 累积器组装出来的块类型——不在任何
  已知集合里，前端也没有渲染分支：真发生一次上下文自动压缩，雷达会指着自己的产物报"未知协议块"
  （现已并入已知并渲染）。`KNOWN_BETAS` 从 `index.html` 上提到 `classifier.py` 并由模板注入：
  清单原先只活在前端，于是唯一会问"这个 beta 是不是新的"的消费者（走 `/api/unknowns` 的 AI）拿不到，
  端点只好退而按频次升序、把长尾当信号。实测：每天顶在最前的"信号"是同样几个**结构性**低频的已知
  特性（`structured-outputs` 只在标题请求带、`token-counting` 只在 token 估算探针带）。beta 维度
  现在分 `new`（不在基线里，真信号）与 `known`（用量分布）。bump `IDX_SCHEMA` 13→14。
- **趋势：加 `burst`、加新鲜度、不再有垃圾桶组。** `sporadic` 原本的定义是"只在一天出现，无论几次"，
  于是 30 天窗口里最大的一次事故——单日 2650 次失败、占窗口失败总数 94%——被标成"零星"，还排在一个
  5 次的组之下。单日 ≥50 次现在标 `burst`。`trend` 只描述形状，新鲜度是正交的另一对字段
  （`days_since_last` + `stale`）——一个两周前就停了的组，只要计数是平的，仍然会被算成 `recurring`。
  上游消息为空或只有一个裸词（`Error`、`timeout`）的组现在按 host 拆分并标 `degenerate`：否则各
  供应商互不相干的失败会并成一个没有诊断价值的组，还因为"跨天出现"排到第一。本机回环 host 单列
  `by_local_loopback`：窗口内 95% 的失败 host 是 `127.0.0.1`（BASE_URL 自指），它不是供应商，混在
  一起会把真实供应商分布彻底淹没。
...
### 文档
- **文档重整：由对账驱动，不是凭手感。** API 契约补上了从来没有过的两节（`/api/grep`、
  `/api/stats`），雷达与趋势两节按新语义重写；`AI_USAGE.md` 补了完整的 CLI 表（17 个子命令，
  此前任何公开文档里都没有清单——文档成形时注意力在 GUI + HTTP，CLI 是后来悄悄长起来的）以及
  「怎么读雷达」一节；架构总览不再抄一份 `IDX_SCHEMA` 当前值，改为指向常量；文档维护策略的
  SSOT 表补了端点 / CLI 子命令 / `KNOWN_BETAS` / schema 数值四行，并删掉了它自己那列会腐化的
  「当前规模」。问题域手册单元 10 补进本轮得到的三个反直觉结论（"未知"里混着三类完全不同的东西、
  "低频"不等于"新出现"、要说"来源"必须算提升度）——这三条对换个 harness 做同类工具的人同样成立。
  自审工作流改用 `trends` 与 `unknowns`，不再是它原先写的逐日 for 循环和人肉翻 header。
