...
用法：uv run python tests/view_selftest.py
...
def tpl_text() -> str:
    return (Path(__file__).resolve().parents[1] / "src" / "templates" / "view.html").read_text(encoding="utf-8")
...
def main() -> None:
    ...
    print("\n[8] 渲染面的静态护栏（不变量④）")
    tpl = (Path(__file__).resolve().parents[1] / "src" / "templates" / "view.html").read_text(encoding="utf-8")
