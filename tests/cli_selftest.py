"""CLI 端到端自测：`uv run python tests/cli_selftest.py`
...
CLI = str(Path(__file__).resolve().parents[1] / "src" / "cli.py")
...
def _fake_record(rid: str, kind: str) -> dict:
    """造一条形似真实抓包的记录（system 三块 + 计费头 + session_id，见 tools/probes/lane_probe.py）。"""
    if kind == "main":
        system = [
